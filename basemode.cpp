...
void BaseMode::baseStart(int file, int type, long nextline, long markposition, int direction)
{
    acquiceType = type;
...
void BaseMode::closeEvent(QCloseEvent *e)
{
    Q_UNUSED(e);
...
void BaseMode::setFieldLabel(int type, double data1, double data2, double lon, double lat)
{
    if (menuStart == false)
    {
        return;
...
void BaseMode::dataSave(int type, double data1, double data2, double lon, double lat)
{
    Q_UNUSED(type);
    Q_UNUSED(lon);
    Q_UNUSED(lat);
...
void BaseMode::simpleEditMenuClose()
{
//    this->showFullScreen();
//    this->show();
    FileDataCache::instance().makeSimpleGrid();
...
void BaseMode::setLineMark(long line, long mark)
{
    if (mark == 0)
    {
        statusValue = 0;
...
void BaseMode::endLineMessageClose()
{
//    this->showFullScreen();
//    this->show();
    WidgetShow::widgetShow(this);
...
void BaseMode::pauseMessageClose()
{
//    this->showFullScreen();
//    this->show();
    WidgetShow::widgetShow(this);
...
void BaseMode::popUpMenuTimeOut(int index)
{
...
void BaseMode::reStartTimerOn(int index)
{
...
void BaseMode::reStart()
{
    dataCollector->start();
    restartTimer.stop();
}

void BaseMode::setPerLineNum(int *perLineNum)
{
    memcpy(this->perLineNum,perLineNum,sizeof(int)*100);
}

void BaseMode::showPopUpMenu(int index)
{
    popUpMenus->setCurrentIndex(index);
    popUpCurrentIndex = index;
}

void BaseMode::setAdjustMenu(bool show)
{
    if (show)
    {
...
void BaseMode::setCenterTrace()
{
    displayPlot->setCenterTrace();
    setAdjustMenu(false);
}

void BaseMode::setFullScale()
{
...
void BaseMode::keyPressEvent(QKeyEvent *e)
{
